...
class LessonInterface:
    ...
    def _create_enhanced_navigation_buttons(self, section_id, topic_id, lesson_id):
        ...
        def on_examples_button_clicked(b):
            ...
            with self.examples_container:
                ...
                try:
                    # ИСПРАВЛЕНО: Передаем контекст курса для обеспечения релевантности примеров
                    course_context = {
                    ...
                    examples = self.content_generator.generate_examples(
                        ...
                        course_context=course_context,  # НОВОЕ: Передаем контекст курса!
                    )
    ...
    def _show_explanation_choice(self):
        ...
        def on_full_explanation_clicked(b):
            ...
            try:
                ...
                # Рабочая кнопка закрытия - перерисовываем весь урок
                close_button = widgets.Button(
                    description="Закрыть объяснение", button_style="primary"
                )

                def on_close_clicked(b):
                    # Полная перерисовка урока
                    clear_output(wait=True)
    ...
    def _hide_explanation_container(self):
        """
        ИСПРАВЛЕНО: Правильно скрывает контейнер объяснений.
        """
        if self.explain_container:
            ...
            with self.explain_container:
                clear_output(wait=True)
            self.logger.info("Контейнер объяснений успешно скрыт")
        else:
